...
export default function AccountDeliveryPreferences({ refreshKey }: { refreshKey?: number }) {
  ...
  const [userAddresses, setUserAddresses] = useState<{ id: string; label: string }[]>([]);
    ...
    async function fetchData() {
      try {
        const [prefRes, addrRes] = await Promise.all([
          fetch("/api/auth/delivery-preferences"),
        ...
        setUserAddresses(addrData.map((a: any) => ({
          id: a.id.toString(),
          label: a.label || "Untitled Address"
        })));
      } catch (err) {
  ...
  const handleRemoveWindow = async (windowId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/auth/delivery-windows/${windowId}`, {
        method: "DELETE",
      });
      if (!response.ok) throw new Error("Failed to delete window.");
      
      setPreferences(prev => ({
        ...prev,
        delivery_windows: prev.delivery_windows.filter(w => w.id !== windowId)
      }));
    } catch (err) {
  ...
  const handleSaveWindow = async (windowId: string) => {
    ...
    try {
      const url = isPendingWindow 
        ? "/api/auth/delivery-windows" 
        : `/api/auth/delivery-windows/${windowId}`;
  ...
  const handleSetDefaultWindow = async (windowId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/auth/delivery-windows/${windowId}/set-default`, {
        method: "POST",
      });
  ...
  const handleSaveNote = async (noteId: string) => {
    ...
    try {
      const url = isPendingNote 
        ? "/api/auth/delivery-notes" 
        : `/api/auth/delivery-notes/${noteId}`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          address_ids: draftNote.addresses?.map(a => a.id) ?? [],
        }),
      });
  ...
  const handleRemoveNote = async (noteId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/auth/delivery-notes/${noteId}`, {
        method: "DELETE",
      });
      ...
      setPreferences(prev => ({
        ...prev,
        delivery_notes: prev.delivery_notes.filter(n => n.id !== noteId)
      }));
    } catch (err) {
  ...
  const handleSetDefaultNote = async (noteId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/auth/delivery-notes/${noteId}/set-default`, {
        method: "POST",
      });
        ...
        <div className="grid gap-3">
          {isInitialLoading ? (
            <div className="flex justify-center py-12">
              <Loader2 className="h-6 w-6 animate-spin text-[#253119]" />
            </div>
      ...
      <section className="rounded-[2rem] border border-[#ddd9c6] bg-white/92 p-5 shadow-[0_22px_50px_rgba(78,95,58,0.08)] sm:p-6 flex flex-col">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm leading-6 text-[#6d7452]">
              Set specific drop-off instructions for different addresses.
            </p>
          </div>
          <div className="shrink-0">
            <button
