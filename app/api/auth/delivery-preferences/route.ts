...
export async function GET() {
  ...
  try {
    const response = await fetch(`${backendBaseUrl}/delivery-preferences`, {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data?.message ?? "Unable to fetch delivery preferences." },
...
export async function PATCH(request: Request) {
  ...
  try {
    const response = await fetch(`${backendBaseUrl}/delivery-preferences/notes`, {
      method: "PATCH",
      headers: {
        Accept: "application/json",
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          message: data?.message ?? "Unable to update delivery notes.",
          errors: data?.errors ?? {},
        },
        { status: response.status }
      );
